...
# Örnek veri seti yükleme (siz kendi veri setinizi buraya yükleyeceksiniz)
X = np.random.rand(100, 2)  # 100 örnek, her biri için 2 özellik
Y = np.random.randint(0, 3, (100, 1))  # 100 örnek için 3 farklı sınıf

# ANFIS modelini başlat
anfis = ANFIS(n_mf=[2, 2], n_outputs=3, problem='C')

# Başlangıç theta değerleri (rastgele veya önceden belirlenmiş değerlerle başlatılabilir)
theta = np.random.rand(anfis.n_var)

# Eğitim döngüsü (burada basit bir döngü gösterilmektedir, gerçek uygulamalarda optimizasyon algoritması kullanılmalıdır)
for i in range(100):  # 100 iterasyon
    J = anfis.create_model(theta, [X, Y])
    print(f"Iterasyon {i}, Hata: {J}")

    # Burada theta'yı güncellemek için bir optimizasyon yöntemi kullanılmalıdır.
    # Bu örnekte, theta'nın nasıl güncelleneceğini göstermek için basit bir düşme yapılıyor.
    theta -= 0.01 * np.random.rand(anfis.n_var)  # Bu satır gerçek bir güncelleme yöntemi değildir!

# Model eğitildikten sonra, tahmin yapmak için:
X_test = np.random.rand(10, 2)  # Test için yeni veri örnekleri
Y_pred = anfis.eval_data(X_test)
# Test veri seti için rastgele gerçek etiketler oluştur
Y_test = np.random.randint(0, 3, (10, 1))  # Test örnekleri için gerçek sınıflar
